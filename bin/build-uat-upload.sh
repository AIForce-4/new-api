...
echo "==> 打包构建文件"
tar czf "$BUILD_DIR/new-api-build-uat.tar.gz" -C "$BUILD_DIR" new-api Dockerfile

if [ ! -s "$BUILD_DIR/new-api-build-uat.tar.gz" ]; then
  echo "打包失败" >&2
  exit 1
fi
...
echo "请输入服务器密码:"
scp "$BUILD_DIR/new-api-build-uat.tar.gz" "$SCP_TARGET"

echo ""
echo "==> 完成"
echo "已上传: ${REMOTE_PATH}/new-api-build-uat.tar.gz"
